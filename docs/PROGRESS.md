...
- ⚠️ **제어-설계 벽 (40 seeds, 외곽루프 보강 후에도)**: ① **s0.5 결합 × 지연** —
  98%@8ms→55%@12ms→5%@20ms (실차 지연대에 마진 없음, model-based 지연보상은 지형에 깨짐)
  ② **s0.75+ 순수 지형 결합** — 지연 0에서도 20% (구 3-seed 맵의 66%는 착시). → **RL 니치 2개**.
