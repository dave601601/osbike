...
- [x] **RL 시뮬 비교 완결** ([상세](progress/rl.md)): 최종 = res_v2@잔차0.5.
      **파라미터 랜덤화 축 = 이 프로젝트의 답.** 두 번 독립 측정했고 결론 일치:
      오차 0%는 smith4 1위, **오차 5%부터 RL 1위, 오차와 함께 격차 확대**
      (18셀 평균 20%: RL 35.4 / smith4 30.1 / base 26.8 ·
       24셀 총합 20%: RL 254±12 / smith4 179 / base 180).
      **"정확 모델이면 고전, 오차 5%↑ 면 model-free"** 가 정본 주장.
      - docs "±5% 붕괴"는 **계통오차(최악방향 단일축)에서 재현됨**(+5% 동률, +10% 역전).
        랜덤·다축 오차에선 붕괴가 아니라 이점의 점진적 소멸로 나타남 — 둘 다 참.
      - **측정 정밀도**: 하네스 1σ≈8런 / **학습 seed 1σ≈23런**(4회 재학습).
        → **셀 단위 주장은 전부 무효** (구 "base 전 셀 격파"·"악화 셀 없음", 그 정정까지).
        정본 RL 명목 수치 = **437±23** (구 447 은 n=1 이라 오차막대 없이 쓴 것).
      - ablation **전부 음성**: pure RL 경사 course-hold 실패(residual 정당화) /
        프레임8 무효(-3) / COMA 해로움(-137, Q(s,0) 외삽) /
        SPR 유의하지 않음(4 seed +13±15 — seed 0 단독 +27·p=0.002 는 오판이었음).
