...
| 저속 직진 균형 | ✅ **v ≥ 0.65 m/s** (사람다움 5/5, 30s+) |
| 선회 (무게추 유도 카운터스티어) | ✅ 완만 선회 slip<3.5° (slew≤7°/s), 40° 정착 |
| 개별 외란 | ✅ 옆경사~5°, 오르막~4°, 마찰 μ0.1(빙판), 범프 ≥5cm |
| **외란 결합 (지연 없이)** | ✅ **~s0.5**(2°/μ0.9/2cm+선회) 100% — 단 s0.75(3°/μ0.6/4cm)는 **25%**로 붕괴 (40 seeds) |
...
- **결합×지연 생존율 맵**: `mm_envelope.py`(**공용 채점 하네스**, 40 seeds, 프로토콜 고정)
  → `envelope_lqr.json` → `mm_plot_envelope.py` → `lqr_envelope_map.png`. RL도 같은 하네스로
  채점해 맵을 겹친다. 지연 격자는 물리스텝(4ms) 정수배만.
...
**B. 제어-설계 한계 — RL 니치 (40-seed 확정):**
- **결합 × 실차 지연 (주 타겟)**: 단독 지연은 40ms까지 OK(detune/Smith predictor로 60ms).
  그러나 s0.5 결합(2°/μ0.9/2cm+선회)에선 **88%@8ms → 48%@12ms → 0%@20ms** (50% 교차
  ~12ms, 완만한 붕괴). 실차 총지연(IMU 10~30ms + 50Hz + 액추에이터)이 이 대역 위.
- **강한 지형 결합 (추가 gap)**: s0.75(3°/μ0.6/4cm)는 **지연 0에서도 25%** — 지연 무관하게
