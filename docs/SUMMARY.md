...
| **외란 결합 (지연 없이)** | ✅ 경사3°+μ0.7+범프3cm **동시** 버팀 (robustness 잘 쌓임) |
...
- **결합×지연 생존율 맵**(`mm_plot_envelope.py`→`lqr_envelope_map.png`): 지형강도×지연.
...
**B. 제어-설계 한계 — RL 니치:**
- **결합 × 실차 지연**: 단독 지연은 40ms까지 OK(detune/Smith predictor로 60ms). 그러나 지형·선회
  결합 시 **~10-15ms에서 붕괴**. 실차 총지연(IMU 10~30ms + 50Hz + 액추에이터)이 이 캡 위.
