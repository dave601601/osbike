...
## 2026-07-07 — 무게추 조향(mass-steering): 완만 선회만 robust, 급선회·반전은 authority 초과

### 결론 (정직한 버전)
조향 모터가 없으므로 **무게추로 조향**: heading 외곽루프가 yaw 오차 → 목표 lean(lean_ref),
내부 4-state LQR이 그 lean 유지 → self-steering 선회. "돌고 싶은 쪽으로 기운다"는 손놓고
타기 원리. **단, 무게추 authority가 작아 완만한 선회만 robust하게 됨:**
- **slew ≤3°/s: robust clean** — 목표 heading 10~60° 어디든 최대 lean 3~4°로 부드럽게 선회.
- slew 10°/s: marginal — lean 20~25° 과도(전복 직전), 살아남긴 함.
- slew ≥20°/s / 방향반전(S커브): **대체로 전복** (무게추가 스트로크 내에서 lean을
  반대로 못 넘김).
